...
// Recording what degraded is what keeps a drawn note from reading as a
// generation bug later.
const report = {
  library: [],
  proposed: [],
  unresolved: [],
...
against a catalog that has fallen a republish behind, settles to `ok: true` as
well — its copy is the first run's copy and clears the same way — and says what
it cost:
