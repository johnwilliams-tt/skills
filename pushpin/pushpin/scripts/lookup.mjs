...
if (has('--help') || has('-h') || (!query && !listOnly)) {
  console.log(
    'usage: node scripts/lookup.mjs [--component|--icon|--token|--style|--annotation]\n' +
      ...
      '  --json   the raw catalog entries, for scripts.',
  );
  process.exit(0);
}
...
function search(names) {
  if (!query) return { matches: names, exact: false };
...
if (kinds.includes('component')) {
  ...
  const { matches } = search(names);
  missPool.push(...names);
  const rows = matches.map((n) => [n, components.components[n]]);
  jsonOut.components = Object.fromEntries(rows);
...
if (kinds.includes('icon')) {
  ...
  jsonOut.icons = Object.fromEntries(rows);
...
if (kinds.includes('token')) {
  ...
  jsonOut.tokens = Object.fromEntries(final.map(([n, { group, value }]) => [group.css(n), value]));
  found += section('tokens', final, total, (name, { group, value }) => renderToken(group, name, value));
}
...
if (kinds.includes('style')) {
  ...
  missPool.push(...names);
  const rows = [
    ...text.filter(([n]) => !query || n.toLowerCase().includes(lower)).map(([n, e]) => [n, { kind: 'text style', e }]),
    ...effect.filter(([n]) => !query || n.toLowerCase().includes(lower)).map(([n, e]) => [n, { kind: 'effect style', e }]),
  ];
  const exact = rows.find(([n]) => n.toLowerCase() === lower);
  const final = exact ? [exact] : rows;
  jsonOut.styles = Object.fromEntries(final.map(([n, { e }]) => [n, e]));
  found += section('styles', final, names.length, (name, { kind, e }) => renderStyle(kind, name, e));
}
...
if (kinds.includes('annotation')) {
  ...
  jsonOut.annotations = Object.fromEntries(rows);
...
console.error(`Nothing in the ${kinds.join(', ')} catalog${kinds.length === 1 ? '' : 's'} matches "${query}".`);
const near = suggest(missPool);
if (near.length) {
  console.error('\nDid you mean:');
